...
$(document).ready(function () {
  ...
  if ($('#attendance').length > 0) {
    var att_table = $('#attendance').DataTable({
      processing: true,
      data: [], // Data will be loaded via an AJAX call
      order: [0, 'desc'],
      columnDefs: [
        {
          targets: 0,
    ...
    $.ajax({
      url: baseurl + 'ajax/?case=LoadingAttendance',
      method: 'GET',
      success: function (response) {
        att_table.clear().rows.add(response.data).draw()
      },
            ...
            for (var i in result.result) {
              $('#all_payheads').append(
                $('<option></option>')
                  ...
                  .text(
                    result.result[i].payhead_name +
                      ' (' +
                      jsUcfirst(result.result[i].payhead_type) +
                      ')'
                  )
                  .addClass(
                    result.result[i].payhead_type == 'earnings'
                      ? 'text-success'
                      : 'text-danger'
                  )
              )
      ...
      $.ajax({
        ...
        success: function (result) {
          if (result.code == 0) {
            ...
            $('#joining_date')
              .val(result.result.joining_date)
              .datepicker('update')
    ...
    var holi_table = $('#holidays').DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          className: 'dt-center',
          defaultContent:
            '<button class="btn btn-success btn-xs editHoliday"><i class="fa fa-edit"></i></button> <button class="btn btn-danger btn-xs deleteHoliday"><i class="fa fa-trash"></i></button>'
        }
      ]
    })
    /* End of Script */
      ...
      $.ajax({
        ...
        success: function (result) {
          if (result.code == 0) {
            ...
            $('#holiday_date')
              .val(result.result.holiday_date)
              .datepicker('update')
    ...
    var pay_table = $('#payheads').DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          className: 'dt-center',
          defaultContent:
            '<button class="btn btn-success btn-xs editPayheads"><i class="fa fa-edit"></i></button> <button class="btn btn-danger btn-xs deletePayheads"><i class="fa fa-trash"></i></button>'
        }
      ]
    })
    /* End of Script */
    ...
    var leave_table = $('#allleaves').DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          className: 'dt-center',
          defaultContent:
            '<button class="btn btn-success btn-xs approveLeave"><i class="fa fa-check"></i></button> <button class="btn btn-danger btn-xs rejectLeave"><i class="fa fa-close"></i></button>'
        }
      ]
    })
