...
<body class="hold-transition skin-blue sidebar-mini">
					...
					<div class="modal-body">
						<div class="row">
							...
							foreach ($months as $key => $month) { ?>
								<div
									class="col-sm-3 <?php echo ($month == date('F') && $years[$key] == date('Y')) ? 'bg-danger' : ''; ?>">
				...
				<div class="modal-content">
					...
					<form method="post" role="form" data-toggle="validator" id="assign-payhead-form">
						<div class="modal-body">
							<div class="row">
								<div class="col-sm-4">
									...
									<button type="button" id="selectHeads" class="btn btn-success btn-xs pull-right"><i
											class="fa fa-arrow-circle-right"></i></button>
									<select class="form-control" id="all_payheads" name="all_payheads[]" multiple
										size="10"></select>
								</div>
								<div class="col-sm-4">
									<label for="selected_payheads">Selected Pay Heads</label>
									<button type="button" id="removeHeads" class="btn btn-danger btn-xs pull-right"><i
											class="fa fa-arrow-circle-left"></i></button>
									<select class="form-control" id="selected_payheads" name="selected_payheads[]"
										data-error="Pay Heads is required" multiple size="10" required></select>
								</div>
								<div class="col-sm-4">
						...
						<div class="modal-footer">
							...
							<button type="submit" name="submit" class="btn btn-primary">Add Pay Heads to
								Employee</button>
						</div>
					</form>
				</div>
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-sm-4">
										...
										<input type="text" class="form-control" name="merital_status"
	...
	<script>
		document.getElementById('download-employee').addEventListener('click', function () {
			...
			XLSX.writeFile(wb, 'employee_list.xls');
		});
		document.getElementById('download-scanner').addEventListener('click', function () {
			// Get the table element
			var table = document.getElementById('employees');

			// Extract table data, excluding the header
			var data = [];
			var rows = table.querySelectorAll('tr');

			// Function to extract and clean ID value
			function extractIdValue(id) {
				// Remove the 'WY' prefix and parse as integer to remove leading zeros
				var numericPart = id.replace(/^WY/, ''); // Remove the 'WY' prefix
				return parseInt(numericPart, 10).toString(); // Convert to integer and back to string to remove leading zeros
			}
			...
			for (var i = 1; i < rows.length; i++) {
				...
				cells.forEach(function (cell, index) {
					if (index === 0 || index === 2) {
						...
						// Extract and clean ID value if it's in the first column
						if (index === 0) {
							cellValue = extractIdValue(cellValue);
						}
						rowData.push(cellValue);
					}
				});
				// Add default values for additional columns
				rowData.push('1'); // Default value for col3
				rowData.push('0'); // Default value for col4
				data.push(rowData);
			}

			// Convert data to a query parameter string
			var encodedData = encodeURIComponent(JSON.stringify(data));
			console.log(encodedData)
			var url = '../register.php?values=' + encodedData;
